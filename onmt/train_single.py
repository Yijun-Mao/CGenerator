...
def main(opt, device_id):
    ...
    if opt.gpt2_params_path is not None:
        ...
        # logger.info("Done.")
        logger.info("Loading CHINESE GPT weights...")
        torchckpt = torch.load(opt.gpt2_params_path, map_location=lambda storage, loc: storage)
        names = []
        arrays = []
        for name, array in torchckpt.items():
    ...
    # valid_stats = trainer.show_valid(
